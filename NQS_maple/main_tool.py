...
# Point directly to your Maple 2026 Apple Silicon Python bindings
...
class EqualityTransformer(ast.NodeTransformer):
    """
    Safely intercepts Python comparisons (A == B) and rewrites them 
    as SymPy equation objects (Eq(A, B)) so they don't evaluate to False.
    """
    ...
    while is_solution == z3.sat:
        ...
        for direction in directions:
            ...
            if str(lambda_var) in [str(m) for m in sub_expr.free_symbols]:
                # ==========================================
                # NATIVE MAPLE 2026 QUANTIFIER ELIMINATION
                # ==========================================
                
                # Format SymPy expression for Maple
                # maple_expr = str(sub_expr).replace("**", "^") 
                
                # 1. Format SymPy expression for Maple syntax
                # maple_expr = str(sub_expr)
                # maple_expr = maple_expr.replace("**", "^") 
                
                # # Fix the logical operators for Maple
                # maple_expr = maple_expr.replace(" & ", " and ")
                # maple_expr = maple_expr.replace(" | ", " or ")
                # maple_expr = maple_expr.replace("~", " not ") # Just in case SymPy uses ~ for NOT
# 1. Helper to recursively convert SymPy AST to Maple Tarski AST
                def sympy_to_maple_logic(expr):
                    from sympy.logic.boolalg import And, Or, Not, Implies, Equivalent
                    from sympy.core.relational import Eq, Ne
                    
                    if isinstance(expr, And):
                        return "And(" + ", ".join(sympy_to_maple_logic(arg) for arg in expr.args) + ")"
                    elif isinstance(expr, Or):
                        return "Or(" + ", ".join(sympy_to_maple_logic(arg) for arg in expr.args) + ")"
                    elif isinstance(expr, Not):
                        return "Not(" + sympy_to_maple_logic(expr.args[0]) + ")"
                    elif isinstance(expr, Implies):
                        return "Implies(" + sympy_to_maple_logic(expr.args[0]) + ", " + sympy_to_maple_logic(expr.args[1]) + ")"
                    elif isinstance(expr, Equivalent):
                        return "Equivalent(" + sympy_to_maple_logic(expr.args[0]) + ", " + sympy_to_maple_logic(expr.args[1]) + ")"
                    elif isinstance(expr, Eq):
                        return sympy_to_maple_logic(expr.lhs) + " = " + sympy_to_maple_logic(expr.rhs)
                    elif isinstance(expr, Ne):
                        return sympy_to_maple_logic(expr.lhs) + " <> " + sympy_to_maple_logic(expr.rhs)
                    else:
                        # Base condition: Replace python power ** with maple ^
                        return str(expr).replace("**", "^")

                # 2. Format SymPy expression into a flawless Maple RationalTarskiFormula
                maple_expr = sympy_to_maple_logic(sub_expr)

                # 3. Run QuantifierElimination
                # We wrap lambda_var in brackets `[{lambda_var}]` as is best practice for Maple QE
                # maple_cmd = f"QuantifierElimination:-QuantifierEliminate(exists([{lambda_var}], {maple_expr}));"
                # maple_result = mpl.execute(maple_cmd)                
                # Run QuantifierElimination
                maple_cmd = f"QuantifierElimination:-QuantifierEliminate(exists({lambda_var}, {maple_expr}));"
                print("maple_cmd:", maple_cmd)
                current_time = timeit.default_timer()
                with open(timer_logs_file,"a") as f:
                    print("TIME_MAPLE_START:", current_time, file=f, flush=True)

                maple_result = mpl.execute(maple_cmd)

                current_time = timeit.default_timer()
                with open(timer_logs_file,"a") as f:
                    print("TIME_MAPLE_FINISH:", current_time, file=f, flush=True)


                # 3. Clean up Maple's output string safely
                pre_cond_str = str(maple_result)

                # 2. Explicitly break the reference to the Maple object
                maple_result = None  # <--- THIS IS THE MITIGATION

                # Convert Maple's not-equal `<>` to Python's `!=` and power `^` to `**`
                pre_cond_str = pre_cond_str.replace("<>", "!=").replace("^", "**")
                
                # Fix Maple's `=` to Python's `==` (safely ignoring `<=`, `>=`, and `!=`)
                pre_cond_str = re.sub(r'(?<![<>!])=(?!=)', '==', pre_cond_str)
                pre_cond_str = pre_cond_str.replace("true", "True").replace("false", "False")

                # ---------------------------------------------------------
                # 4. AST Transformation: Safely convert `A == B` into `Eq(A, B)`
                # ---------------------------------------------------------
                tree = ast.parse(pre_cond_str, mode='eval')
                tree = EqualityTransformer().visit(tree)
                pre_cond_str = ast.unparse(tree)

                logger.debug(f"PRECOND_{iteration_index}: {pre_cond_str}")


                # 5. Convert String back to SymPy logic
                ...
                # # Clean up string for SymPy parsing
                # pre_cond_str = str(maple_result).replace(" and ", " & ").replace(" or ", " | ").replace("true", "True").replace("false", "False")
                # pre_cond_str = re.sub(r'(?<![<>!])=(?!=)', '==', pre_cond_str)

                # logger.debug(f"PRECOND_{iteration_index}: {pre_cond_str}")

                # # Convert String back to SymPy logic
                # sympy_pre_cond = sympy.sympify(pre_cond_str)
                # logger.debug(f"PRECOND_SYMPY_{iteration_index}: {sympy_pre_cond}")
