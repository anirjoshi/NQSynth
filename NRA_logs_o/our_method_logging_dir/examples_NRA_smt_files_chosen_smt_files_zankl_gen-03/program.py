...
def post_condition(delta:sympy.Rational, a:sympy.Rational):
	# (0 <= delta) & (a**2 - 2 <= delta) & (2 - a**2 <= delta)

	post_cond =  And(LessThan(Integer(0), Symbol('delta')), LessThan(Add(Pow(Symbol('a'), Integer(2)), Integer(-2)), Symbol('delta')), LessThan(Add(Integer(2), Mul(Integer(-1), Pow(Symbol('a'), Integer(2)))), Symbol('delta')))
