...
def pre_condition_0(b:sympy.Rational,delta:sympy.Rational):
	#(b**2 + delta - 2 >= 0) & (-2*b + delta + 1 > 0) & (-b**2 + delta + 2 >= 0)

	pre_cond = And(GreaterThan(Add(Pow(Symbol('b'), Integer(2)), Symbol('delta'), Integer(-2)), Integer(0)), StrictGreaterThan(Add(Mul(Integer(-1), Integer(2), Symbol('b')), Symbol('delta'), Integer(1)), Integer(0)), GreaterThan(Add(Mul(Integer(-1), Pow(Symbol('b'), Integer(2))), Symbol('delta'), Integer(2)), Integer(0)))
...
if __name__=="__main__":
	...
	if pre_condition_0(b=b,delta=delta)==True:
		...
		all_vals['a'] = Add(Symbol('lambda_var_0'), Integer(1))
