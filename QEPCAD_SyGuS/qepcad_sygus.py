...
from chat_gpt_translator5 import write_program
...
if __name__=="__main__":
    ...
    parser = argparse.ArgumentParser(description='stage2', usage='%(prog)s [-h] [options] post_condition', formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    ...
    parser.add_argument('--sygus_command', help='command to execute sygus', type=str, default="cvc5 --sygus-qe-preproc --lang=sygus2")

    parser.add_argument('--output_program', help='name of output program', type=str, default="gus_program.py")

    parser.add_argument('--cvc_op', help='name of output program from sygus', type=str, default="sygus_op.txt")
    ...
    output_program_file = args.output_program
    sygus_command = args.sygus_command
    cvc_op = args.cvc_op
    ...
    if "smt" in ip_file_name.split(".")[-1]:
        ...
        qf_vars = post_cond_sympy_expr.free_symbols
        qf_vars = [str(m) for m in qf_vars if str(m) in op_vars]

        write_sage_file(sage_expr=sage_expr,vars_quantify_out=qf_vars,all_vars=ip_vars+op_vars,sage_file=sage_file)
        ...
        s.add(pre_cond_z3)
        smt2_data = s.to_smt2()
        # print("smt2_data:", smt2_data, pre_cond_z3.sexpr()=="true")
    ...
    if pre_cond_z3.sexpr()=="true" or pre_cond_z3.sexpr()=="false":
        pre_condition_z3_sexpr = pre_cond_z3.sexpr()
    else:
        pre_condition_z3_sexpr = None
    
    print("synthesizing sygus file....")
    synthesize_sygus_file.synthesize_sygus_file(post_condition_file=post_condition_file, pre_condition_file=pre_condition_file, ip_vars=ip_vars, op_vars=op_vars, output_file=output_sygus_file, pre_condition_z3_sexpr=pre_condition_z3_sexpr)

    current_time = timeit.default_timer()
    with open(timer_logs_file,"a") as f:
        print("TIME_SYGUS_START:", current_time, file=f, flush=True)

    os.system(f"{sygus_command} {output_sygus_file} > {cvc_op}")

    current_time = timeit.default_timer()
    with open(timer_logs_file,"a") as f:
        print("TIME_SYGUS_STOP:", current_time, file=f, flush=True)
    
    current_time = timeit.default_timer()
    with open(timer_logs_file,"a") as f:
        print("TIME_PROG_SYN_START:", current_time, file=f, flush=True)
    write_program(pre_condition_sympy=weakest_pre_condition, sygus_synthesized_function_file=cvc_op, ip_vars=ip_vars, op_vars=op_vars, output_program_name=output_program_file)
    current_time = timeit.default_timer()
    with open(timer_logs_file,"a") as f:
        print("TIME_PROG_SYN_STOP:", current_time, file=f, flush=True)

    with open(timer_logs_file,"a") as f:
        print("PROGRAM_SYNTHESIZED", file=f, flush=True)
